...
| 开测日期 | 2026-05-29 |
| 功能变更摘要 | 新增 CreateOrder：幂等创建 + 库存校验 + 风控拦截 |
| 涉及服务 | order-svc |
| 触及对外入口 | 1 个（CreateOrder） |
| 自测形态 | 单场景平铺 |
| plan 版本 | v2（最近变更：3.2 幂等命中预期调整，详见 [plan 版本历史](./plan.md#版本历史)）|

> 环境与观测工具（在哪测、用什么发起/查）属执行期，记在 [round_N.md 自测环境段](./round1.md#1-元信息)，不在此处。
...
#### 基础测试（依据 [SKILL 基础测试模板](../../../self-auto-test/SKILL.md#基础测试模板))
