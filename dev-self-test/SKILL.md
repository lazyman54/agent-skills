...
|---|---|---|
| 仅 1 场景 | **平铺**（同目录）| `docs/dev-self-test/{branch}/{README,plan,round1,defects}.md` |
| ≥ 2 场景 | **场景子目录** + 顶层 README | `docs/dev-self-test/{branch}/README.md` + `docs/dev-self-test/{branch}/{scene}/{plan,round1,defects}.md` |

多场景目录树示意：

```
docs/dev-self-test/feature-order-create/
...
定路线靠两样：**意图**（用户措辞，只是*信号*）+ **状态**（事实）。探测状态三件事：①有无 plan（看 `docs/dev-self-test/{branch}/`）②基线 = 父分支（复用 `git-mr-target-branch` 确定法，不确定问用户）③`git diff <父分支>...HEAD` 取全量改动点。
